...
import { DashboardRowCellStatus, Dashboard, DashboardRow, DashboardRowCell, PlayerBase, PlayerResponse, DashboardRowStatus, GamePlayer } from '@app/models';
...
export class PlayerUtils {
      ...
      return <Dashboard>{
        rows: dashboard.map(row => {
          ...
          for (let index = 0; index < GAME_DASHBOARD_LIMIT_CELL; index++) {
            rowValue = row.filter(rowValue => index * 10 < rowValue && rowValue < (index + 1) * 10);
  ...
  createGamePlayerFromPlayer(player: PlayerResponse): GamePlayer {
    return <GamePlayer> { id: player.id, name: player.name };
  }
}
